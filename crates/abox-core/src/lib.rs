//! `abox-core` — Domain logic for abox.
//!
//! This crate contains the core domain types, port traits (hexagonal
//! architecture), and adapter implementations for workspace management,
//! sandbox runtime lifecycle, and credential policy evaluation.
//!
//! # Architecture
//!
//! The crate follows hexagonal (ports & adapters) architecture:
//!
//! - **Ports** define trait interfaces: [`workspace::WorkspacePort`],
//!   [`runtime::SandboxRuntimePort`]
//! - **Adapters** implement those traits: [`adapters::git2_workspace::Git2Workspace`],
//!   [`adapters::microsandbox::MicrosandboxRuntime`]
//! - **Domain types** live in [`config`], [`policy`], [`protocol`], [`error`]
//! - **Orchestration** happens in [`sandbox::SandboxOrchestrator`]

pub mod adapters;
pub mod audit;
pub mod ca;
pub mod command_broker;
pub mod config;
pub mod error;
pub mod mcp_oauth;
pub mod policy;
pub mod project;
pub mod protocol;
pub mod request_broker;
pub mod runtime;
pub mod sandbox;
pub mod services;
pub mod util;
pub mod workspace;
