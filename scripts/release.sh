...
#   - Attestation stamps at .abox-attestations/{runtime,smoke}.json
...
Steps performed:
  ...
  2.  Verify attestation stamps (.abox-attestations/{runtime,smoke}.json)
...
Quality checks (fmt, clippy, test), the live runtime e2e suite, and the
agent smoke tests are covered by the attestation stamps. Run
...
if [[ "$SKIP_ATTESTATION" == "1" ]]; then
    ...
    for stamp_name in runtime smoke; do
...
# Note: quality checks (cargo fmt/clippy/test) and e2e suites are
# covered by the attestation stamps (runtime, smoke) verified above.
